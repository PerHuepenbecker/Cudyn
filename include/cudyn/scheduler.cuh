...
namespace Cudyn::Scheduler{
    ...
    template <typename TaskFunctor>
    __global__ void genericIrregularKernelBatch(uint64_t total_tasks, uint64_t count_blocks, TaskFunctor f){
        ...
        while (true) {
        uint64_t t = atomicAdd((unsigned long long*)&counter, chunk_size);
            if (t >= block_tasks) break;

            #pragma unroll
            for (int i = 0; i < chunk_size; ++i) {
                if (t + i >= block_tasks) break;
                f(block_start_index + t + i);
    }
}
...
namespace Cudyn::Launcher{
    template <typename TaskFunctor>
    __host__ void launch(GridConfiguration::KernelConfig kernelConfig, TaskFunctor f, Scheduler::KernelType type = Scheduler::KernelType::STANDARD, int deviceId = 0){
